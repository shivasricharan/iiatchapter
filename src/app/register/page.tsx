                  ...
                  <AnimatePresence>
                    {form.memberType === "other-chapter" && (
                      <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }}
                        className="glass gold-border rounded-2xl p-6 overflow-hidden space-y-5">

                        {/* Step A: Are you an IIA Telangana Chapter Member? */}
                        <div>
                          <label className="text-xs uppercase tracking-widest block mb-3" style={{ color: "#c9a227" }}>
                            Are you an IIA Telangana Chapter Member?
                          </label>
                          <div className="flex gap-3">
                            {[{ label: "Yes", val: true }, { label: "No", val: false }].map(({ label, val }) => (
                              <button key={label} type="button"
                                ...
                                style={{ background: isTelanganaQ === val ? "rgba(201,162,39,0.2)" : "rgba(255,255,255,0.04)", border: isTelanganaQ === val ? "1px solid rgba(201,162,39,0.7)" : "1px solid rgba(255,255,255,0.1)", color: isTelanganaQ === val ? "#c9a227" : "rgba(245,245,240,0.6)" }}>
                                {label}
                              </button>
                            ))}
                          </div>
                        ...
                        {/* If No to Telangana → Step B: Are you an IIA Member? */}
                        <AnimatePresence>
                          {isTelanganaQ === false && (
                            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden space-y-4">
                                ...
                                <div className="flex gap-3">
                                  {[{ label: "Yes", val: true }, { label: "No", val: false }].map(({ label, val }) => (
                                    <button key={label} type="button"
                                      ...
                                      className="flex-1 py-3 rounded-xl font-semibold text-sm transition-all"
                                      style={{ background: isNationalMemberQ === val ? "rgba(201,162,39,0.2)" : "rgba(255,255,255,0.04)", border: isNationalMemberQ === val ? "1px solid rgba(201,162,39,0.7)" : "1px solid rgba(255,255,255,0.1)", color: isNationalMemberQ === val ? "#c9a227" : "rgba(245,245,240,0.6)" }}>
                                      {label}
                                    </button>
                                  ))}
                                </div>
                              ...
                              {/* If Yes → IIA membership number from any chapter */}
                              <AnimatePresence>
